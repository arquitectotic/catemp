@extends('adminlte::page')

@section('template_title')
    Registro
@endsection

@section('content')
    <br><div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <div style="display: flex; justify-content: space-between; align-items: center;">

                            <span id="card_title">
                                {{ __('Registro') }}
                            </span>

                            <div class="float-right">
                                <a href="{{ route('registros.create') }}" class="btn btn-primary btn-sm float-right"  data-placement="left">
                                {{ __('Nuevo') }}
                                </a>
                            </div>
                        </div>
                    </div>
                    @if ($message = Session::get('success'))
                        <div class="alert alert-success">
                            <p>{{ $message }}</p>
                        </div>
                    @endif

                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                <thead class="thead">
                                    <tr>
                                        <th>No</th>
                                        
										<th>Fecha Registro</th>
										<th>Hora Registro</th>
										<th>Asesor</th>
										<th>Cliente</th>
										<th>Empresa</th>
										<th>Servicio</th>
										<th>Poliza</th>
										<th>Canal</th>
										<th>Contacto</th>
										<th>Zona Contacto</th>
										<th>Accion</th>
										<th>Etapa</th>
										<th>Fecha Proxima Accion</th>

                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($registros as $registro)
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            
											<td>{{ $registro->fecha_registro }}</td>
											<td>{{ $registro->hora_registro }}</td>
											<td>{{ $registro->user_id }}</td>
											<td>{{ $registro->id_cliente }}</td>
											<td>{{ $registro->id_empresa }}</td>
											<td>{{ $registro->id_servicio }}</td>
											<td>{{ $registro->id_poliza }}</td>
											<td>{{ $registro->id_canal }}</td>
											<td>{{ $registro->id_contacto }}</td>
											<td>{{ $registro->zona_contacto }}</td>
											<td>{{ $registro->id_accion }}</td>
											<td>{{ $registro->id_etapa }}</td>
											<td>{{ $registro->fecha_proxima_accion }}</td>

                                            <td>
                                                <form action="{{ route('registros.destroy',$registro->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('registros.show',$registro->id) }}"><i class="fa fa-fw fa-eye"></i> Ver</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('registros.edit',$registro->id) }}"><i class="fa fa-fw fa-edit"></i> Editar</a>
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-fw fa-trash"></i> Eliminar</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                {!! $registros->links() !!}
            </div>
        </div>
    </div>
@endsection
